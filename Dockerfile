...
CMD ["sh", "-c", "npx prisma db push --accept-data-loss && if [ \"$DISABLE_WORKER\" != \"true\" ]; then echo 'Starting background worker...'; npx tsx src/worker.ts & fi; echo 'Starting Next.js server...'; exec node server.js"]
