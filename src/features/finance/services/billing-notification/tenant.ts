...
export async function notifyPaymentConfirmed(paymentId: string): Promise<void> {
  try {
    ...
    const templateVars = {
      ...
      amount: formatCurrency(payment.amount),
      invoiceType: type,
      employeeCount: String((tenant as any).employeeCount || 0),
...
📦 Tipe: ${type}${
  meta?.type !== "AI_QUOTA" ? `
👥 Jumlah Karyawan: ${(tenant as any).employeeCount || 0}` : `
      ...
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f8fafc; padding: 24px; border: 1px solid #e2e8f0;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
            ...
            ${meta?.type !== "AI_QUOTA" 
              ? `<tr><td style="padding: 8px 0; color: #64748b;">Jumlah Karyawan</td><td style="padding: 8px 0; font-weight: 600;">${(tenant as any).employeeCount || 0}</td></tr>`
    ...
    for (const owner of owners) {
      if (owner.user.phone && cfg.enablePaymentConfirmed) {
        const wavioVars = {
          ...
          "4": type,
          "5": String((tenant as any).employeeCount || 0),
