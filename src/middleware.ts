...
function addSecurityHeaders(response: NextResponse): NextResponse {
  ...
  response.headers.set(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://static.cloudflareinsights.com https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: blob: https:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' ws: wss: https://cloudflareinsights.com; frame-src 'self' https://challenges.cloudflare.com; frame-ancestors 'none'"
  )
  return response
}
...
export default async function middleware(request: NextRequest) {
  ...
  if (isMainDomain) {
    ...
    const systemRoutes = ["/admin", "/super-admin", "/affiliate", "/login", "/register", "/daftarkan-sekolah", "/api", "/invoice", "/mitra-afiliasi", "/privacy-policy"]
    ...
    const isProtected = pathname.startsWith("/admin") || pathname.startsWith("/super-admin") || pathname.startsWith("/affiliate") || pathname.startsWith("/ortu") || pathname.startsWith("/guru") || pathname.startsWith("/member")
    ...
    if (isAuthPage && session) {
      if (session.user?.isSuperAdmin) {
        ...
        return addSecurityHeaders(NextResponse.redirect(new URL("/guru", request.url)))
