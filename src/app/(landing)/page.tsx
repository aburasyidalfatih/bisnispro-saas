      ...
      <nav className="glass sticky top-0 z-50 border-b">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <Link href="/" className="flex items-center gap-2">
            <Image src={normalizeImageUrl(appLogo) || appLogo} alt={`${platformName} Logo`} width={120} height={32} className="h-8 w-auto object-contain" unoptimized />
              ...
              {[...activeTenants, ...activeTenants, ...activeTenants, ...activeTenants].map((tenant, idx) => {
                ...
                return (
                  <div key={`${tenant.id}-${idx}`} className="flex items-center gap-3 shrink-0 opacity-70 hover:opacity-100 transition-opacity grayscale hover:grayscale-0 cursor-default">
                    {tenant.logo ? (
                      <Image src={normalizeImageUrl(tenant.logo) || tenant.logo} alt={tenant.name} width={48} height={48} className="h-10 w-10 md:h-12 md:w-12 object-contain rounded-full border bg-white p-1" unoptimized />
                    ) : (
