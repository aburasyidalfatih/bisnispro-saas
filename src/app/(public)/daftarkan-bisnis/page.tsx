...
import { BisnisProfileSection } from "./_components/business-profile-section"
...
function RegisterBusinessForm() {
  ...
  useEffect(() => {
    ...
    if (ref) {
      ...
      document.cookie = `bisnispro_ref=${ref}; path=/; max-age=${30 * 24 * 60 * 60}`
    } else {
      const storedRef = localStorage.getItem('bisnispro_ref') || getCookie('bisnispro_ref')
      if (storedRef) {
        activeRef = storedRef
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsChecking(true)
      try {
        const res = await fetch(`/api/public/check-subdomain?slug=${form.businessSlug}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.adminPosition) {
      toast({ title: "Gagal", description: "Silakan pilih Jabatan di Bisnis.", variant: "destructive" })
      return
    }
    ...
    const originalRef = typeof window !== "undefined" ? localStorage.getItem('bisnispro_ref') : null
    const cookieRef = getCookie('bisnispro_ref')
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmitted(true)
        trackMetaEvent('Lead')
        toast({ title: "Berhasil!", description: `Pendaftaran website bisnis berhasil dikirim.` })
      } else {
        ...
        <Card className="max-w-md w-full glass border-0 text-center p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <CardDescription className="text-base">
              Pendaftaran website bisnis <strong>{form.businessName}</strong> berhasil dikirim.
            </CardDescription>
          ...
          {csPhone ? (
            <Button 
              ...
              onClick={() => {
                const text = encodeURIComponent(`halo admin saya sudah isi formulir pengajuan website untuk bisnis saya ${form.businessName} dengan email ${form.adminEmail}`)
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-muted/30 py-12 px-4">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-foreground">Daftarkan Bisnis Anda</h1>
          <p className="text-muted-foreground text-lg max-w-lg mx-auto leading-relaxed">
            Bergabunglah dengan {tenantCount ? <span className="font-semibold text-primary">{tenantCount}</span> : "ratusan"} bisnis lainnya dalam transformasi digital manajemen bisnis.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <BisnisProfileSection 
          ...
          <p className="text-center text-xs text-muted-foreground">
            Dengan mendaftar, Anda menyetujui <a href="#" className="underline">Syarat & Ketentuan</a> Platform BisnisPro.
          </p>
