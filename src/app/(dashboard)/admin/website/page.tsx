...
export default function WebsiteOverviewPage() {
  ...
  const sections: StatItem[] = [
    {
      label: "Profil Bisnis",
      desc: "Isi logo, alamat, dan nomor telepon perusahaan.",
    ...
    {
      label: "Sambutan",
      desc: "Tambahkan kata sambutan dari Kepala Perusahaan.",
    ...
    {
      label: "Galeri Foto",
      desc: "Dokumentasi visual lingkungan & acara perusahaan.",
    ...
    {
      label: "Aset & Kantor",
      desc: "Daftar kantor cabang dan aset operasional.",
      value: data?._count?.offices ? `${data._count.offices} lokasi` : "Belum ada",
      icon: <Building2 className="h-5 w-5" />,
      status: getColStatus(data?._count?.offices),
      href: `${base}/offices`,
    },
    {
      label: "Tim Profesional",
      desc: "Profil pimpinan dan anggota tim inti perusahaan.",
      value: data?._count?.teamMembers ? `${data._count.teamMembers} profil` : "Belum ada",
      icon: <Users className="h-5 w-5" />,
      status: getColStatus(data?._count?.teamMembers),
      href: `${base}/team`,
    },
    {
      label: "Portofolio Klien",
      desc: "Studi kasus dan daftar proyek yang telah diselesaikan.",
      value: data?._count?.portfolios ? `${data._count.portfolios} proyek` : "Belum ada",
      icon: <Award className="h-5 w-5" />,
      status: getColStatus(data?._count?.portfolios),
      href: `${base}/portfolio`,
    },
    {
      label: "Testimoni",
      desc: "Ulasan positif dan testimoni dari klien.",
      value: data?._count?.testimonials ? `${data._count.testimonials} testimoni` : "Belum ada",
      icon: <Handshake className="h-5 w-5" />,
      status: getColStatus(data?._count?.testimonials),
      href: `${base}/testimonials`,
    },
    {
      label: "Layanan Bisnis",
      desc: "Daftar layanan atau produk unggulan yang ditawarkan.",
      value: data?._count?.services ? `${data._count.services} layanan` : "Belum ada",
      icon: <BookOpen className="h-5 w-5" />,
      status: getColStatus(data?._count?.services),
      href: `${base}/services`,
    },
    {
