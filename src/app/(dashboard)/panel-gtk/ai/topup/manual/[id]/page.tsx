...
export default async function ManualTopUpAIPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!payment || payment.plan !== "AI_TOKEN_USER") {
    redirect("/panel-gtk/ai")
  }
  ...
  return (
    <div className="pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-xl mx-auto md:mt-8 md:rounded-3xl md:overflow-hidden md:border md:shadow-2xl md:shadow-indigo-500/10 bg-background">
      {/* Header Premium */}
      <div className="bg-gradient-to-br from-amber-500 to-amber-700 pt-10 pb-20 px-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Link href="/panel-gtk/ai" className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-white/20 text-white hover:bg-white/30 backdrop-blur-md transition-colors mb-6 cursor-pointer">
          ...
          <p className="text-amber-100 mt-1 text-sm">Transfer manual untuk pembelian Token AI.</p>
        </div>
      </div>
            ...
            <CardContent className="p-6">
               ...
               {isPaid ? (
                  <div className="text-center p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                     ...
                     <p className="text-xs text-emerald-600/80 mt-1">Token AI Anda sudah masuk dan dapat digunakan.</p>
                  </div>
               ) : isPending ? (
                  <div className="text-center p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
                     ...
                     <p className="text-xs text-amber-600/80 mt-1">Bukti transfer Anda sedang dicek oleh Super Admin Platform. Token akan otomatis bertambah setelah diverifikasi.</p>
                  </div>
               ) : (
