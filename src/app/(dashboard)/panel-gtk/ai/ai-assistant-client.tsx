  ...
  return (
    <div className="flex h-[calc(100vh-140px)] gap-4 w-full">
      ...
      <div className="hidden lg:flex w-72 flex-col gap-4">
        <Card className="border-0 shadow-lg glass flex-1 flex flex-col overflow-hidden">
          <CardHeader className="py-4 border-b border-border/30 bg-white/50 dark:bg-black/20">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm flex items-center gap-2">
                <History className="h-4 w-4 text-primary" />
                Riwayat Chat
              </CardTitle>
              <Button variant="ghost" size="icon" className="h-8 w-8 text-primary" onClick={startNewChat}>
                ...
                {chatSessions.length === 0 ? (
                  ...
                  chatSessions.map((s) => (
                    <Button 
                      ...
                      className={`w-full justify-start text-left p-4 h-auto rounded-none transition-colors text-sm ${activeSessionId === s.id ? 'bg-primary/10 border-l-4 border-primary' : 'hover:bg-muted/50 border-l-4 border-transparent'}`}
                    >
                      <div className="flex flex-col gap-1 w-full overflow-hidden">
                        <div className="font-medium truncate w-full text-foreground">{s.title}</div>
      ...
      <div className="flex-1 flex flex-col">
        <Card className="border-0 shadow-lg glass flex-1 flex flex-col relative overflow-hidden">
          <CardHeader className="border-b border-border/30 bg-white/50 dark:bg-black/20 py-3 backdrop-blur-xl z-10 relative flex-row justify-between items-center space-y-0">
            <div className="flex items-center gap-3">
              <div className="bg-gradient-to-br from-primary to-indigo-600 p-2 rounded-lg shadow-md shadow-primary/20">
                <Bot className="h-5 w-5 text-white" />
              </div>
              <div>
                <CardTitle className="text-base font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-indigo-600">Asisten AI Pro</CardTitle>
