...
export function UploadProofForm({ paymentId, invoiceId }: { paymentId: string, invoiceId: string }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // We can safely reuse the wallet topup manual upload API because it's generic for any Payment record
      const res = await fetch(`/api/ortu/invoices/${invoiceId}/pay/manual`, {
        method: "POST",
        body: formData
      })
      
      if (!res.ok) throw new Error("Gagal mengupload bukti pembayaran")
