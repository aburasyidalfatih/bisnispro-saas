...
export default async function OrtuPrintInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Security check: ensure the current user is a parent of the student in the invoice
  if (!invoice || !invoice.student?.parents?.length) {
    return <div className="p-10 text-center">Tagihan tidak ditemukan atau Anda tidak memiliki akses.</div>
  }
