...
import { getTenantLayoutData, getTenantKlien } from "@/features/tenant/services/tenant-modular.service"
import { getPublicBasePath } from "@/lib/utils/public-path"
import { OptimizedImage } from "@/components/ui/optimized-image"
import { cn, normalizeImageUrl } from "@/lib/utils"
import { KlienSubmissionForm } from "./_components/klien-submission-form"
import { buildDynamicBreadcrumbs } from "@/lib/utils/breadcrumbs"


export default async function KlienPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const tenant = await getTenantLayoutData(slug)
  
  if (!tenant) notFound()

  const klienData = await getTenantKlien(slug)
  const klien = (klienData as any)?.klien || []
  const base = await getPublicBasePath(slug)

  // Custom Theme rendering
  if (tenant.customThemeId && tenant.customTheme?.klienHtml) {
    const { renderCustomTheme } = await import("@/app/site/[slug]/_themes/custom-renderer")
    const rendered = renderCustomTheme({
      templateHtml: tenant.customTheme.klienHtml,
      layoutHtml: tenant.customTheme.layoutHtml,
      customCss: tenant.customTheme.customCss,
      customJs: tenant.customTheme.customJs,
      context: { tenant: { ...tenant, klien }, base, settings: tenant.settings || {} },
    })
  ...
  return (
    <div className="bg-background min-h-screen">
      {/* ── HERO SECTION ── */}
      <PageHeader
        title={(tenant.settings as any)?.labels?.klien?.sectionTitle || "Jejak Langkah Klien"}
        description={(tenant.settings as any)?.labels?.klien?.sectionSubtitle || "Melihat kontribusi dan kesuksesan para proyek selesai kami yang kini telah berkiprah di berbagai bidang dan institusi ternama."}
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/klien", (tenant.settings as any)?.labels?.klien?.sectionTitle || "Jejak Langkah Klien")}
      />
        ...
        <div className="space-y-12">
            <div className="flex items-center gap-4 mb-8">
               ...
               <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                 <div>
                    ...
                    <p className="text-muted-foreground">Testimoni tulus dari para klien tentang perjalanan mereka.</p>
                 </div>
                 <KlienSubmissionForm tenantId={tenant.id} />
               </div>
            </div>
            ...
            {klien.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-primary font-bold">Proyek Selesai Tahun {item.graduationYear}</p>
                        <div className="flex flex-col gap-2 mt-1">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-1">
                            {item.instagram && (
                              <a href={item.instagram} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="Instagram">
                            ...
                            {item.facebook && (
                              <a href={item.facebook} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="Facebook">
                            ...
                            {item.youtube && (
                              <a href={item.youtube} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="YouTube">
                                <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"/><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/></svg>
                              </a>
                            )}
                            {item.tiktok && (
                              <a href={item.tiktok} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="TikTok">
                            ...
                            {item.linkedin && (
                              <a href={item.linkedin} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="LinkedIn">
                            ...
                            {item.twitter && (
                              <a href={item.twitter} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="Twitter / X">
                            ...
                            {item.pinterest && (
                              <a href={item.pinterest} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-primary transition-colors" title="Pinterest">
                    ...
                    <div className="mt-8 pt-8 border-t border-border/50 flex items-center justify-between">
                       <div className="flex items-center gap-4">
                          ...
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Verified Klien</span>
                       </div>
              ...
              <div className="text-center py-20 bg-muted/20 rounded-[2.5rem] border border-dashed border-border">
                ...
                <p className="text-muted-foreground mt-2">Daftar testimoni klien sedang dalam proses pengumpulan.</p>
              </div>
            )}
          </div>
      </section>
         ...
         <div className="relative z-10 max-w-4xl mx-auto px-4">
            <h2 className="text-3xl md:text-5xl font-black mb-8 leading-tight">
              {tenant.tagline ? (
                ...
                <>Mendidik dengan Hati, <br/> Mencetak Generasi Berportofolio</>
              )}
            </h2>
