...
export function TeamHighlight({ staff, labels, basePath = "" }: TeamHighlightProps & { labels?: any }) {
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sky-500/10 text-sky-600 text-xs font-bold tracking-wider uppercase mb-4">
              ...
              {l.sectionTitle || "Tim Profesional"}
            </div>
            <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-foreground to-primary">
              {l.sectionTitle || "Tim Profesional Kami"}
            </h2>
            <p className="text-muted-foreground text-sm md:text-base max-w-xl">
              {l.sectionSubtitle || "Tim pengajar profesional dan berdedikasi yang siap membimbing klien menuju kesuksesan."}
            </p>
          </div>
          <Link href={`${basePath}/tim`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline whitespace-nowrap">
        ...
        <div className="relative flex overflow-hidden group/slider -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 py-4">
          ...
          >
            {displayStaff.map((member, i) => (
              <Link key={`${member.id}-${i}`} href={`${basePath}/tim/${member.id}`} className="group/card text-center flex-shrink-0 w-[140px] md:w-[160px] max-w-full">
                <div className="relative mx-auto w-28 h-28 md:w-32 md:h-32 rounded-2xl overflow-hidden border-2 border-border bg-muted mb-4 shadow-sm group-hover/card:shadow-xl group-hover/card:border-primary/30 transition-all duration-300">
                  {member.imageUrl ? (
                    <Image src={normalizeImageUrl(member.imageUrl)!} alt={`Tim Profesional: ${member.name}`} fill className="object-cover object-top group-hover/card:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 128px, 128px" />
                  ) : (
          ...
          >
            {displayStaff.map((member, i) => (
              <Link key={`${member.id}-clone-${i}`} href={`${basePath}/tim/${member.id}`} className="group/card text-center flex-shrink-0 w-[140px] md:w-[160px] max-w-full">
                <div className="relative mx-auto w-28 h-28 md:w-32 md:h-32 rounded-2xl overflow-hidden border-2 border-border bg-muted mb-4 shadow-sm group-hover/card:shadow-xl group-hover/card:border-primary/30 transition-all duration-300">
                  {member.imageUrl ? (
                    <Image src={normalizeImageUrl(member.imageUrl)!} alt={`Tim Profesional: ${member.name}`} fill className="object-cover object-top group-hover/card:scale-110 transition-transform duration-500" sizes="(max-width: 768px) 128px, 128px" />
                  ) : (
