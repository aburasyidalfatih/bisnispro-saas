...
export function PortfolioSection({ achievements, labels, basePath = "" }: PortfolioSectionProps & { labels?: any }) {
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-foreground to-primary">
              {l.achievements ? `${l.achievements} Membanggakan` : "Portofolio Terbaik"}
            </h2>
            <p className="text-muted-foreground text-sm md:text-base max-w-xl">
              Deretan pencapaian klien dan perusahaan kami di berbagai kompetisi dan ajang bergengsi.
            </p>
          </div>
          <Link href={`${basePath}/portofolio`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline whitespace-nowrap">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayed.map((achievement) => {
            ...
            return (
              <Link
                key={achievement.id}
                href={`${basePath}/portofolio/${achievement.slug || achievement.id}`}
                ...
                <div className="flex items-start gap-4">
                  {achievement.imageUrl ? (
                    <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 border bg-white shadow-sm">
                      <Image src={normalizeImageUrl(achievement.imageUrl)!} alt={`Portofolio: ${achievement.title}`} fill className="object-cover group-hover:scale-110 transition-transform duration-500" sizes="64px" />
                    </div>
                  ) : (
