...
export function LatestUpdates({ events, posts, basePath }: LatestUpdatesProps) {
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-background rounded-[2rem] p-6 md:p-8 border shadow-sm flex flex-col h-full min-w-0">
            <div className="flex items-center justify-between mb-8 pb-4 border-b">
              ...
              <Link href={`${basePath}/event`} className="text-xs font-semibold text-muted-foreground hover:text-primary flex items-center gap-1 transition-colors">
            ...
            <div className="space-y-6 flex-1">
              {agenda.length > 0 ? agenda.map((item, idx) => (
                <Link key={idx} href={`${basePath}/event/${item.slug || item.id}`} className="flex gap-4 group cursor-pointer">
