...
export function OfficesSection({ facilities, labels, basePath = "" }: OfficesSectionProps & { labels?: any }) {
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            ...
            <p className="text-muted-foreground text-sm md:text-base max-w-xl">
              Kantor/Lokasi modern dan lengkap untuk mendukung proses belajar mengajar yang optimal.
            </p>
          </div>
          <Link href={`${basePath}/kantor`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
          {displayed.map((facility, idx) => {
            ...
            return (
              <Link
                key={facility.id}
                href={`${basePath}/kantor/${facility.slug || facility.id}`}
              ...
              >
                {facility.imageUrl ? (
                  <Image 
                    ...
                    alt={`Kantor/Lokasi: ${facility.name}`} 
                    fill 
