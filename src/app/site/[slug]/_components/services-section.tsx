...
export function ServicesSection({ programs, labels, basePath = "" }: ServicesSectionProps & { labels?: any }) {
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-foreground to-primary">
              {l.sectionTitle || "Layanan Kami"}
            </h2>
            <p className="text-muted-foreground text-sm md:text-base max-w-xl">
              {l.sectionSubtitle || "Berbagai layanan yang dirancang untuk membekali klien dengan kompetensi profesional dan siap menghadapi dunia kerja."}
            </p>
          </div>
          <Link href={`${basePath}/layanan`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline whitespace-nowrap">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayed.map((program, idx) => {
            ...
            return (
              <Link
                key={program.id}
                href={`${basePath}/layanan/${program.slug || program.id}`}
