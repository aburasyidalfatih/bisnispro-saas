...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const description = `Daftar layanan dan akademik unggulan di ${tenant.name}`
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      <PageHeader
        ...
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/layanan", (tenant.settings as any)?.labels?.programs?.sectionTitle || "Program Unggulan")}
      />
              ...
              >
                ...
                <div className="p-8 md:p-10 flex-1 flex flex-col">
                  ...
                  <p className="text-muted-foreground line-clamp-2">
                    {prog.description ? prog.description.replace(/<[^>]*>?/gm, '') : "Program pendidikan yang dirancang khusus untuk mengoptimalkan potensi intelektual dan keterampilan klien secara komprehensif."}
                  </p>
                  <Link href={`${base}/layanan/${prog.slug || prog.id}`} className="flex items-center gap-2 text-primary font-bold text-sm">
