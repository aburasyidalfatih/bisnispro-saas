...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = `Tim Profesional Kami`
  const description = `Profil dan direktori Tim Profesional Kami (GTK) di ${tenant.name}`
...
export default async function GTKPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const gtkStats = (tenant.settings as any)?.gtkStats?.length > 0 ? (tenant.settings as any).gtkStats : [
    { icon: "Users", label: "Total Tim & Staf", value: totalStaff > 0 ? `${totalStaff}+` : "—" },
    { icon: "GraduationCap", label: "Proyek Selesai S1/S2", value: totalStaff > 0 ? "98%" : "—" },
    { icon: "BookOpen", label: "Rasio Tim:Klien", value: "1:20" },
    { icon: "Award", label: "Tim Berportofolio", value: totalStaff > 3 ? `${Math.round(totalStaff * 0.3)}` : "—" },
  ]
  ...
  const menuPath = findMenuPath(tenant.websiteMenus || [], "/tim")
  const dynamicBreadcrumbs = menuPath 
    ? menuPath.map(m => ({ label: m.label }))
    : [
        { label: "Beranda" },
        { label: (tenant.settings as any)?.labels?.staff?.sectionTitle || "Tim Profesional Kami" }
      ]
  ...
  return (
    <div className="bg-background min-h-screen">
      {/* ── HERO SECTION ── */}
      <PageHeader
        title={(tenant.settings as any)?.labels?.staff?.sectionTitle || "Tim Profesional Kami"}
        ...
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/tim", (tenant.settings as any)?.labels?.staff?.sectionTitle || "Tim Profesional Kami")}
      />
        ...
        <section className="py-20 bg-muted/30 border-b border-border/50">
          <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-border flex flex-col md:flex-row gap-12 items-center relative">
              <Link href={`${base}/tim/${principalSlug}`} className="relative h-64 w-64 md:h-80 md:w-80 rounded-[2.5rem] overflow-hidden shadow-2xl shrink-0 group">
                <OptimizedImage 
      ...
      <section className="py-12 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {teachers.length > 0 ? (
          ...
          <div className="text-center py-20 bg-muted/20 rounded-3xl border border-dashed border-border">
            ...
            <h3 className="text-xl font-bold">Data Tim Sedang Diperbarui</h3>
