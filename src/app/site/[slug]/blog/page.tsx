...
export default async function BeritaPage({ 
  ...
  const pageDesc = typeFilter === 'PENGUMUMAN' ? ((tenant.settings as any)?.labels?.pengumuman?.sectionSubtitle || "Informasi resmi dan edaran penting dari perusahaan.") : ((tenant.settings as any)?.labels?.news?.sectionSubtitle || `Ikuti informasi terkini mengenai kegiatan, portofolio, dan pengumuman di ${tenant.name}.`)
  ...
  return (
    <div className="bg-background min-h-screen pb-12">
      <PageHeader
        ...
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/blog", pageTitle)}
      />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-10 md:mb-14">
          ...
          {activeCategories.map((cat) => {
            ...
            const href = `${base}/blog?category=${cat.slug}`
            return (
              <Link
                key={cat.id}
                href={href}
        ...
        {posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 bg-white/50 rounded-[3rem] border border-dashed border-border/60">
             ...
             <p className="text-muted-foreground max-w-sm text-center">Informasi terbaru akan segera dipublikasikan oleh pihak perusahaan. Pantau terus halaman ini.</p>
          </div>
        ) : (
          <div className="space-y-12">
            ...
            {posts[0] && (
              ...
              >
                <div className="w-full lg:w-3/5 aspect-video relative overflow-hidden bg-muted">
            ...
            {posts.length > 1 && (
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-16">
                  {page > 1 && (
                    <Link href={`${base}/blog?page=${page - 1}${typeFilter ? `&type=${typeFilter}` : ''}`} className="px-4 py-2 border rounded-xl hover:bg-muted font-medium">
                      Sebelumnya
                    </Link>
                  )}
