...
export default async function AgendaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="bg-background min-h-screen pb-12">
      {/* ── HERO SECTION ── */}
      <PageHeader
        title={(tenant.settings as any)?.labels?.agenda?.sectionTitle || "Agenda & Acara Perusahaan"}
        ...
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/event", (tenant.settings as any)?.labels?.agenda?.sectionTitle || "Agenda & Acara Perusahaan")}
      />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 bg-white/50 rounded-[3rem] border border-dashed border-border/60">
             ...
             <p className="text-muted-foreground max-w-sm text-center">Jadwal acara dan kegiatan perusahaan akan segera diperbarui di sini.</p>
          </div>
        ) : (
          <div className="max-w-4xl mx-auto space-y-6">
            {events.map((event: any) => (
              <Link 
                key={event.id} 
                href={`${base}/event/${event.slug || event.id}`}
              ...
              >
                ...
                <div className="flex-1 p-8 md:p-10 flex flex-col justify-center">
                  ...
                  <div className="flex flex-wrap gap-x-8 gap-y-3 text-sm text-muted-foreground mb-6 font-medium">
                     ...
                     <div className="flex items-center gap-2 bg-muted/50 px-3 py-1.5 rounded-lg">
                        <MapPin className="h-4 w-4 text-primary" /> {event.location || "Area Perusahaan"}
                     </div>
                  </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16">
         <div className="bg-primary/5 rounded-3xl p-8 border border-primary/10 flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
               ...
               <p className="text-sm text-muted-foreground">Silakan hubungi bagian keklienan atau panitia terkait untuk informasi lebih lanjut.</p>
            </div>
