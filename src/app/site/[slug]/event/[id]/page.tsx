...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  return {
    ...
    alternates: {
      canonical: `/event/${event.slug || event.id}`,
    },
  }
}
  ...
  return (
    <div className="bg-background min-h-screen pb-16">
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "location": {
              "@type": "Place",
              "name": event.location || "Perusahaan",
              "address": {
                "@type": "PostalAddress",
                "addressCountry": "ID"
              }
      ...
      <div className="bg-muted/30 pt-6 pb-10 border-b">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <SiteBreadcrumbs 
            ...
            currentItemUrl={`/event/${event.slug || event.id}`}
          />
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 md:mt-12">
          ...
          <div className="md:col-span-2 grid sm:grid-cols-2 gap-4">
            <div className="bg-background rounded-2xl p-5 border border-border/50 flex items-start gap-4 shadow-sm">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{event.location || "Area Perusahaan"}</p>
              </div>
            </div>
            <div className="bg-background rounded-2xl p-5 border border-border/50 flex items-start gap-4 shadow-sm">
              <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
        ...
        {/* Share Buttons */}
        <ShareButtons 
          url={`https://${tenant.domain || tenant.slug + '.' + rootDomain}/event/${event.slug || event.id}`} 
          title={event.title}
          tenantId={tenant.id}
        />
      </article>
