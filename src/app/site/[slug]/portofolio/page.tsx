...
import { PortofolioList } from "./portofolio-list"
import { buildDynamicBreadcrumbs } from "@/lib/utils/breadcrumbs"


export default async function PortofolioPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="bg-background min-h-screen">
      {/* ── HERO SECTION ── */}
      <PageHeader
        title={(tenant.settings as any)?.labels?.achievements?.sectionTitle || "Portofolio & Penghargaan"}
        description={(tenant.settings as any)?.labels?.achievements?.sectionSubtitle || "Membanggakan dan Inspiratif. Catatan perjalanan klien dan institusi dalam meraih keunggulan di berbagai bidang."}
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/portofolio", (tenant.settings as any)?.labels?.achievements?.sectionTitle || "Portofolio & Penghargaan")}
      />

      {/* ── MAIN CONTENT ── */}
      <section className="py-20 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <PortofolioList achievements={achievements} base={base} />
      </section>
      ...
      <section className="py-24 bg-dark text-white relative overflow-hidden bg-slate-900">
         ...
         <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold italic leading-tight mb-8">
               "Portofolio bukanlah akhir, melainkan awal dari tanggung jawab yang lebih besar untuk terus berkarya."
            </h2>
