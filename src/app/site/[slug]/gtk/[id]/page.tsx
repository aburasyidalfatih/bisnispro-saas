        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-center md:items-start">
             ...
             <div className="relative shrink-0 group">
               <div className="absolute inset-0 bg-gradient-to-tr from-primary to-blue-500 rounded-full blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-700"></div>
               <div className="w-48 h-48 md:w-56 md:h-56 relative rounded-full overflow-hidden border-[6px] border-white/80 shadow-2xl bg-white backdrop-blur-xl">
             ...
             <div className="flex-1 text-center md:text-left pt-2 md:pt-6">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                   <a href={staff.email ? `mailto:${staff.email}` : "#"} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/60 text-slate-700 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all font-semibold text-sm shadow-sm group/btn">
                   ...
                   {staff.instagram && (
                     <a href={staff.instagram} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/60 text-slate-400 hover:bg-pink-500 hover:text-white hover:border-pink-500 transition-all shadow-sm group/icon">
                   ...
                   {staff.facebook && (
                     <a href={staff.facebook} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/60 text-slate-400 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm group/icon">
                   ...
                   {staff.youtube && (
                     <a href={staff.youtube} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/60 text-slate-400 hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm group/icon">
                       <svg className="h-4 w-4 text-slate-400 group-hover/icon:text-white transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"/><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/></svg>
                     </a>
                   )}
                   {staff.tiktok && (
                     <a href={staff.tiktok} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/60 text-slate-400 hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm group/icon">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 md:mt-8">
        <div className="grid md:grid-cols-3 gap-10 items-start">
           ...
           <div className="space-y-8 sticky top-32">
              <div className="relative rounded-[2.5rem] p-8 shadow-2xl border border-slate-800 bg-slate-900 overflow-hidden group">
                 {/* Decorative elements */}
                 <div className="absolute top-0 right-0 w-40 h-40 bg-primary/20 rounded-full blur-[50px] -translate-y-1/2 translate-x-1/3 transition-transform duration-700 group-hover:scale-110"></div>
                 <div className="absolute bottom-0 left-0 w-32 h-32 bg-blue-500/20 rounded-full blur-[40px] translate-y-1/3 -translate-x-1/3"></div>
                 
                 <h4 className="font-black text-xl mb-8 text-white flex items-center gap-3 relative z-10">
                    Informasi Akademik
                 </h4>
                 
                 <ul className="space-y-8 relative z-10">
                    <li className="flex items-start gap-5 group/item">
                       <div className="h-14 w-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-blue-400 shrink-0 shadow-inner group-hover/item:bg-blue-500/20 group-hover/item:text-blue-300 group-hover/item:border-blue-500/30 transition-all duration-300">
                          <BookOpen className="h-6 w-6" />
                       </div>
                       <div className="flex-1">
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1.5">Mata Pelajaran</p>
                          <p className="font-bold text-white text-base leading-tight">{staff.subject || "Guru Kelas / Umum"}</p>
                       </div>
                    </li>
                    ...
                    <li className="flex items-start gap-5 group/item">
                       <div className="h-14 w-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-primary shrink-0 shadow-inner group-hover/item:bg-primary/20 group-hover/item:border-primary/30 transition-all duration-300">
                          <GraduationCap className="h-6 w-6" />
                       </div>
                       <div className="flex-1">
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1.5">Pendidikan</p>
                          <p className="font-bold text-white text-base leading-tight">{staff.education || "S1 Pendidikan"}</p>
                       </div>
                    </li>
                 </ul>
              </div>
