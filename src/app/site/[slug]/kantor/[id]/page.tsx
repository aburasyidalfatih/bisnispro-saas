...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  return {
    ...
    description: (facility.description ? facility.description.replace(/<[^>]*>?/gm, '') : `Kantor/Lokasi ${facility.name} di ${tenant.name}`),
    alternates: {
      canonical: `/kantor/${facility.slug || facility.id}`,
    },
  }
}
      ...
      <div className="bg-muted/30 pt-8 pb-12 border-b">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <SiteBreadcrumbs 
            ...
            basePath={base}
            targetUrl="/kantor"
            fallbackLabel={(tenant.settings as any)?.labels?.facilities?.sectionTitle || "Kantor/Lokasi"}
            currentItemName={facility.name}
            currentItemUrl={`/kantor/${facility.slug || facility.id}`}
          />
          ...
          <div className="flex items-center gap-3 mb-4 mt-6">
             <div className="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-bold uppercase tracking-widest flex items-center gap-1.5">
               <Building2 className="h-3.5 w-3.5" /> Kantor/Lokasi Utama
             </div>
          </div>
           ...
           <div className="lg:col-span-2">
              ...
              <div className="prose prose-lg max-w-none text-muted-foreground leading-relaxed">
                 {facility.description ? (
                    <div className="whitespace-pre-wrap prose prose-slate max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(facility.description, { ADD_TAGS: ["iframe", "video", "source"], ADD_ATTR: ["allow", "allowfullscreen", "frameborder", "scrolling", "controls"] }) }} />
                 ) : (
                    <p className="italic">Tidak ada penjelasan lebih detail mengenai kantor ini.</p>
                 )}
              </div>
           </div>
                 ...
                 <li className="flex items-start gap-4">
                    <div className="h-10 w-10 rounded-xl bg-background shadow-sm border border-border/50 flex items-center justify-center text-primary shrink-0"><Tag className="h-5 w-5" /></div>
                     <div>
                        ...
                        <p className="font-semibold text-foreground mt-1">{facility.category || "Kantor/Lokasi Umum"}</p>
                     </div>
                  </li>
                  <li className="flex items-start gap-4">
