...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title,
    description,
    alternates: { canonical: "/kantor" },
    openGraph: {
      title,
      description,
      url: `${domainUrl}/kantor`,
    }
  }
}
...
export default async function Kantor/LokasiPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Jika perusahaan menggunakan Custom Theme dan menyediakan template kantor
  ...
  return (
    <div className="bg-background min-h-screen">
      {/* ── HERO SECTION ── */}
      <PageHeader
        title={(tenant.settings as any)?.labels?.facilities?.sectionTitle || "Kantor & Lokasi"}
        description={(tenant.settings as any)?.labels?.facilities?.sectionSubtitle || "Sarana dan prasarana pendukung pendidikan berkualitas untuk kenyamanan seluruh klien."}
        breadcrumbs={buildDynamicBreadcrumbs(tenant.websiteMenus || [], "/kantor", (tenant.settings as any)?.labels?.facilities?.sectionTitle || "Kantor & Lokasi")}
      />
      ...
      <section className="py-16 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {facilities.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            {facilities.map((facility: any, index: number) => {
              ...
              return (
                <Link 
                  key={facility.id} 
                  href={`${base}/kantor/${facility.slug || facility.id}`}
                  className={cn(
                ...
                >
                  ...
                  <div className="relative mt-auto p-8 flex flex-col justify-end text-white z-10">
                    ...
                    <p className="text-white/80 text-sm leading-relaxed mb-0 line-clamp-2 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                      {facility.description ? facility.description.replace(/<[^>]*>?/gm, '') : "Klik untuk melihat informasi selengkapnya mengenai sarana prasarana perusahaan ini."}
                    </p>
                  </div>
                </Link>
              )
          ...
          <div className="text-center py-20 bg-muted/20 rounded-3xl border border-dashed border-border">
            ...
            <h3 className="text-xl font-bold">{(tenant.settings as any)?.labels?.empty?.facilitiesTitle || "Data Kantor/Lokasi Belum Tersedia"}</h3>
            <p className="text-muted-foreground mt-2 max-w-sm mx-auto">{(tenant.settings as any)?.labels?.empty?.facilitiesDesc || "Maaf, saat ini kami belum memperbarui daftar kantor perusahaan secara detail di website ini."}</p>
          </div>
        )}
      </section>
