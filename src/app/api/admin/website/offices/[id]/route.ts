...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { name, slug, address, city, phone, email, mapUrl, imageUrl, isHeadquarter, sortOrder } = body
    
    const result = await db.office.update({
      where: { id: resolvedParams.id, tenantId },
      data: { name, slug, address, city, phone, email, mapUrl, imageUrl, isHeadquarter, sortOrder }
    })
