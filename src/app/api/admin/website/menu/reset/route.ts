...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await tenantDb.websiteMenu.updateMany({
      where: {},
      data: { parentId: null }
    })
    ...
    const beranda = await tenantDb.websiteMenu.create({ data: { tenantId, label: "Beranda", url: "/", isSystem: true, order: 0 } })
    const profil = await tenantDb.websiteMenu.create({ data: { tenantId, label: "Profil Sekolah", url: "/profil", isSystem: false, order: 1 } })
    const informasi = await tenantDb.websiteMenu.create({ data: { tenantId, label: "Informasi", url: "/berita", isSystem: false, order: 2 } })
    const galeri = await tenantDb.websiteMenu.create({ data: { tenantId, label: "Galeri", url: "/gallery", isSystem: false, order: 3 } })
    const kontak = await tenantDb.websiteMenu.create({ data: { tenantId, label: "Kontak", url: "/contact", isSystem: false, order: 4 } })

    await tenantDb.websiteMenu.createMany({ data: [
