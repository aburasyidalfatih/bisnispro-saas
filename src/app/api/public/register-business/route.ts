...
const registerSchoolSchema = z.object({
  businessName: z.string().min(3, "Nama bisnis minimal 3 karakter").max(200),
  businessSlug: z
    .string()
    .min(3, "Subdomain minimal 3 karakter")
    .max(50)
    .regex(/^[a-z0-9-]+$/, "Slug hanya boleh huruf kecil, angka, dan strip"),
  businessType: z.enum(["UMKM", "Startup", "Corporate", "Agency", "Professional", "Freelancer"]).optional().default("UMKM"),
  ...
  employeeCount: z.coerce.number().min(1, "Jumlah karyawan harus lebih dari 0"),
...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      province, regency, adminName, adminEmail, adminPhone, adminPosition, password, address, logo, employeeCount, referralCode,
    ...
    // Cek duplikasi di pengajuan aplikasi (Email atau Slug)
    ...
    if (existingApp) {
      ...
      if (isExpiredPending) {
        ...
        if (existingApp.businessSlug === businessSlug) {
          return NextResponse.json({ error: "Subdomain sudah diajukan sebelumnya" }, { status: 400 })
        }
        if (existingApp.adminEmail === adminEmail) {
          return NextResponse.json({ error: "Email ini sedang dalam proses pengajuan bisnis lain" }, { status: 400 })
        }
      }
    }
    ...
    if (existingUser) {
      return NextResponse.json({ error: "Email ini sudah terdaftar sebagai pengguna BisnisPro" }, { status: 400 })
    }
    ...
    if (isInstant) {
      ...
      const platformName = process.env.NEXT_PUBLIC_APP_NAME || "BisnisPro"
      ...
      const emailHtml = `
        <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #4f46e5, #7c3aed); padding: 24px; border-radius: 12px 12px 0 0; color: white; text-align: center;">
            ...
            <p style="margin: 4px 0 0; opacity: 0.9;">Untuk mengaktifkan website bisnis Anda</p>
          </div>
          <div style="background: #f8fafc; padding: 24px; border: 1px solid #e2e8f0; line-height: 1.6; text-align: center;">
            ...
            <p>Terima kasih telah mendaftarkan <strong>${businessName}</strong> di ${platformName}.</p>
            <p>Satu langkah lagi! Silakan klik tombol di bawah ini untuk memverifikasi alamat email Anda. Setelah email terverifikasi, subdomain bisnis Anda akan <strong>langsung aktif</strong> seketika.</p>
            <div style="margin: 32px 0;">
              <a href="${verifyUrl}" style="background-color: #4f46e5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">Verifikasi & Aktifkan Bisnis</a>
            </div>
      ...
      `
      await sendEmail(adminEmail, "Verifikasi Email Pendaftaran Bisnis", emailHtml).catch((e: any) => logger.error("Failed sending verify email", e))
    ...
    logger.error("Registration error", error, { path: "/api/public/register-business" })
